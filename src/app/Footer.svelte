<script>
    import { page, email, onSubscribe } from "../utility.js"
        ...
        <div class="space-y-8 flex items-center flex-col">
            ...
            <div class="space-y-1 flex items-center flex-col">
                ...
                <div class="flex">
                    ...
                    <button class="py-2 px-4 bg-a" on:click={onSubscribe}>Subscribe</button>
                </div>
            </div>
        </div>
                ...
                <div class="space-x-2 flex justify-end mt-auto p-2">
                    {#each $page.website.socialAccounts as { profileUrl, serviceName }}
                        {#if ['instagram', 'youtube', 'facebook'].includes(getName(serviceName))}
                            <a href={profileUrl} target="_blank" class="icon w-6">
                                {#if 'instagram' == getName(serviceName)}
                                    <svg
                                        class="icon"
                                        role="img"
                                        viewBox="0 0 24 24"
                                ...
                                {:else if 'youtube' == getName(serviceName)}
                                    <svg
                                        class="icon"
                                        role="img"
                                ...
                                {:else if 'facebook' == getName(serviceName)}
                                    <svg
                                        class="icon"
                                        role="img"
                                        xmlns="http://www.w3.org/2000/svg"
